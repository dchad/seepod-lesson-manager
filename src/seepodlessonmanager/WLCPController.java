...
public class WLCPController extends JPanel implements GenericController
{
  ...
  public synchronized void wlcpDownloadFinished()
  {
     ...
     javax.swing.SwingUtilities.invokeLater(new Runnable()
     {
        public void run()
        {
           DirectoryTree treePanel = seePodConfig.getTreePanel(); //Must put on the main event thread otherwise Substance UI fails on tree UI update --- cannot call from a thread
           if (treePanel != null)
           {
              treePanel.update(); //update the directory tree
           }
        }
     });
  }
